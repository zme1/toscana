...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                <div class="svgContainer">
                    <div class="svg">
                        ...
                        <line x1="{$xScale *55}" y1="-{$yHeight}" x2="{$xScale * 55}" y2="0" stroke="black" stroke-width="0.25" stroke-dasharray="10, 5"/> -->
                                ...
                                <text x="{$xScale *10}" y="20" text-anchor="middle">10</text>
                                <line x1="{$xScale *20}" y1="-{$yHeight}" x2="{$xScale * 20}" y2="0"
                                    stroke="white" stroke-width="0.5" stroke-dasharray="10, 5"/>
                                <text x="{$xScale *20}" y="20" text-anchor="middle">20</text>
                                <line x1="{$xScale *30}" y1="-{$yHeight}" x2="{$xScale * 30}" y2="0"
                                    stroke="white" stroke-width="0.5" stroke-dasharray="10, 5"/>
                                <text x="{$xScale *30}" y="20" text-anchor="middle">30</text>
                                <line x1="{$xScale *40}" y1="-{$yHeight}" x2="{$xScale * 40}" y2="0"
                                    stroke="white" stroke-width="0.5" stroke-dasharray="10, 5"/>
                                <text x="{$xScale *40}" y="20" text-anchor="middle">40</text>
                                <line x1="{$xScale *50}" y1="-{$yHeight}" x2="{$xScale * 50}" y2="0"
                                    stroke="white" stroke-width="0.5" stroke-dasharray="10, 5"/>
                                <text x="{$xScale *50}" y="20" text-anchor="middle">50</text>
                                <line x1="{$xScale *60}" y1="-{$yHeight}" x2="{$xScale * 60}" y2="0"
                                    stroke="white" stroke-width="0.5" stroke-dasharray="10, 5"/>
                                <text x="{$xScale *60}" y="20" text-anchor="middle">60</text>
    ...
    <xsl:template match="teiCorpus/teiCorpus" mode="svg" xmlns="http://www.w3.org/2000/svg">
        ...
        <xsl:variable name="barWidthAcc"
            select="(count(descendant::list[@type = 'applicants']/item[not(persName[@role = 'rejected'])][not(note)])) * $xScale"/>
        ...
        <text class="toggle" id="textToggle{$date}" x="0" y="-{$yPos - $barHeight div 2 - 5}"
            text-anchor="middle" style="color: white;"><xsl:value-of select="$date"/></text>
    </xsl:template>
        ...
        <div class="table" id="accettati{$date}">
            <table>
                ...
                <tr>
                    ...
                    <td class="in">
                        <xsl:value-of
                            select="string-join(descendant::list[@type = 'applicants']/item/persName[not(note)], ', ')"
                        />
                    </td>
                    <td class="out">
                        <xsl:value-of
