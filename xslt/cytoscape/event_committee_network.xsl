...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:apply-templates select="//list[@type='committee'][not(@subtype='regolamento' or @subtype='investigazione')]"/>
    </xsl:template>
    <xsl:template match="list[@type='committee'][not(@subtype='regolamento' or @subtype='investigazione')]">
