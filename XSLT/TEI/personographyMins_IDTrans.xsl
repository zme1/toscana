        ...
        <persName>
            ...
            <roleName><xsl:apply-templates select="roleName"/>
                ...
                <xsl:for-each select="$minutes//TEI[descendant::profileDesc//person[substring-after(persName/@ref, '#')=$currentID]]">
                    <xsl:variable name="minutesMatch" select="current()//descendant::profileDesc//person[substring-after(persName/@ref, '#')=$currentID]"/>
                    <roleName type="{$minutesMatch/roleName/@type}" role="{$minutesMatch/roleName/@role}">
                   <date when="{$minutesMatch//ancestor::teiHeader//publicationStmt/date/@when}"/>
