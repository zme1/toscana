            ...
            <g transform="translate(60,{$yLength + 30})">
                <xsl:for-each select="$persons">
                    ...
                    <xsl:for-each select="$dates">
                        ...
                        <xsl:if test="$root//act[@type='officer' and @ref eq $currentPerson and date/@when = $currentDate]">
                            <line x1="{$currentPersonPos * $xScale}" y1="-{$currentDatePos * $yScale}" x2="{$currentPersonPos * $xScale}" y2="-{$currentDatePos * $yScale - $yScale}" stroke="black" stroke-width="2"/>
                            <line x1="{$currentPersonPos * $xScale + $xScale}" y1="-{$currentDatePos * $yScale}" x2="{$currentPersonPos * $xScale + $xScale}" y2="-{$currentDatePos * $yScale - $yScale}" stroke="black" stroke-width="2"/>
                        </xsl:if>
                ...
                <xsl:for-each select="//year">
                    ...
                    <xsl:variable name="prevMeetingCount" as="xs:integer" select="count(preceding-sibling::year/meeting)"/>
                    <xsl:variable name="meetingTotal" as="xs:integer" select="$meetingCount + $prevMeetingCount"/>
                    <line x1="8.892" y1="-{$meetingTotal * $yScale}" x2="{$xLength + 8.892}" y2="-{$meetingTotal * $yScale}" stroke="black" stroke-width="2"/>
                    <text x="-25" y="-{(($meetingCount div 2) + $prevMeetingCount) * $yScale}" text-anchor=""><xsl:value-of select="@when"/></text>
                </xsl:for-each>
